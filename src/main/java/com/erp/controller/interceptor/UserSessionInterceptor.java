...
public class UserSessionInterceptor extends HandlerInterceptorAdapter{
  ...
  */  
    ...
    public boolean preHandle(HttpServletRequest request,    
        ...
        log.info("---------------进入拦截器执行顺序: 1、preHandle--------------");    
        String requestUri = request.getRequestURI();  
        String contextPath = request.getContextPath();  
        String url = requestUri.substring(contextPath.length());  
        
        User user =  (User) httpSession.getAttribute(Constant.LOGIN_UER_KEY);   
        log.info("-----------------session的user={}",user);
        if (url.equals("/user/login") || url.equals("/user/logOut")) {
            return true;
        } else {
            if(null == user){  
              log.info("Interceptor：user 失效了");  
              return false;  
            }
        }
        return true;     
    ...
    public void postHandle(HttpServletRequest request,    
        ...
        log.info("-------------controller请求后执行顺序: 2、postHandle-------------");    
    }    
